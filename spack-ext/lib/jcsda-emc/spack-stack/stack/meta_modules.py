...
def setup_meta_modules():
    ...
    # Get all specs and determine compilers and mpi providers
    ...
    # MAYBE KEEP FOR NOW UNTIL WE KNOW BETTER WHAT IS NEEDED AND WHAT NOT.
    ...
    for compiler in compilers:
        ...
        if "environment" in compiler.extra_attributes.keys():
            ...
            if "append_path" in compiler.extra_attributes["environment"].keys():
                for env_name in compiler.extra_attributes["environment"]["append_path"]:
                    env_values = compiler.extra_attributes["environment"]["append_path"][env_name]
                    substitutes["ENVVARS"] += append_path_command(
                        module_choice, env_name, env_values
                    )
            # prepend_path
            if "prepend_path" in compiler.extra_attributes["environment"].keys():
                for env_name in compiler.extra_attributes["environment"]["prepend_path"]:
                    env_values = compiler.extra_attributes["environment"]["prepend_path"][env_name]
                    substitutes["ENVVARS"] += prepend_path_command(
                        module_choice, env_name, env_values
                    )
            # set
            if "set" in compiler.extra_attributes["environment"].keys():
                for env_name in compiler.extra_attributes["environment"]["set"]:
                    env_values = compiler.extra_attributes["environment"]["set"][env_name]
                    substitutes["ENVVARS"] += setenv_command(
                        module_choice, env_name, env_values
                    )
            # unset
            if "unset" in compiler.extra_attributes["environment"].keys():
                for env_name in compiler.extra_attributes["environment"]["unset"]:
                    env_values = compiler.extra_attributes["environment"]["unset"][env_name]
                    substitutes["ENVVARS"] += unsetenv_command(
                        module_choice, env_name, env_values
                    )
            # *DH
