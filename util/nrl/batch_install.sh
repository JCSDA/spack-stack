...
# Currently, we cannot run the tests when submitting to compute nodes
if [[ ${SPACK_STACK_SUBMIT_TO_SCHEDULER} == "true" && ${SPACK_STACK_RUN_TESTS} == "true" ]]; then
  echo "ERROR, submitting to compute nodes and running the tests is not supported yet"
  exit 1
fi
...
is_rhel9() {
    cat /etc/redhat-release | grep -qi "Red Hat Enterprise Linux release 9"
}
if is_wsl; then
  if is_rhel9; then
    SPACK_STACK_BATCH_HOST="nrl-rhel9"
  fi
fi

case ${SPACK_STACK_BATCH_HOST} in
  atlantis)
    SPACK_STACK_BATCH_COMPILERS=("oneapi@=2026.0.0" "gcc@=13.4.0" "clang@=22.1.7")
    ...
    ;;
  blueback)
    SPACK_STACK_BATCH_COMPILERS=("oneapi@=2026.0.0" "gcc@=14.3.0")
    ...
    ;;
  narwhal)
    SPACK_STACK_BATCH_COMPILERS=("oneapi@=2026.1.0" "gcc@=13.3.0")
    SPACK_STACK_BATCH_TEMPLATES=("neptune-dev" "unified-dev" "cylc-dev")
    SPACK_STACK_MODULE_CHOICE="tcl"
    SPACK_STACK_BOOTSTRAP_MIRROR="/p/app/projects/NEPTUNE/spack-stack/bootstrap-mirror"
    SPACK_STACK_CARGO_MIRROR="/p/app/projects/NEPTUNE/spack-stack/cargo-mirror"
  ...
  navy-aws)
    ...
    ;;
  nrl-rhel9)
    SPACK_STACK_BATCH_COMPILERS=("oneapi@=2026.1.0" "gcc@=14.2.1")
    SPACK_STACK_BATCH_TEMPLATES=("neptune-dev" "unified-dev" "cylc-dev")
    SPACK_STACK_MODULE_CHOICE="tcl"
    SPACK_STACK_BOOTSTRAP_MIRROR="/home/dev/prod/spack-stack/bootstrap-mirror"
    SPACK_STACK_CARGO_MIRROR="/home/dev/prod/spack-stack/cargo-mirror"
    ;;
  blackpearl)
    ...
    ;;
  bounty)
    SPACK_STACK_BATCH_COMPILERS=("oneapi@=2025.3.0" "gcc@=14.2.1" "clang@=22.1.0")
    SPACK_STACK_BATCH_TEMPLATES=("neptune-dev" "neptune-dev-llvm" "unified-dev" "cylc-dev")
    SPACK_STACK_MODULE_CHOICE="tcl"
    SPACK_STACK_BOOTSTRAP_MIRROR="/home/dom/prod/spack-bootstrap-mirror"
    SPACK_STACK_CARGO_MIRROR="/home/dom/prod/spack-cargo-mirror"
...
function run_interactive_job() {
  ...
  case ${host} in
    atlantis)
      module load slurm
      salloc --exclusive --nodes=1 --ntasks-per-node=${tpn} --time=${walltime} --mem=0 srun --ntasks=1 ${script} "${script_arg}"
      module unload slurm
      ;;
    blueback)
      salloc --exclusive --nodes=1 --ntasks-per-node=${tpn} --time=${walltime} --qos=serial --account=NRLMR03795YH2 --mem=0 srun --ntasks=1 ${script} "${script_arg}"
      ;;
    narwhal)
      salloc --exclusive --nodes=1 --ntasks-per-node=${tpn} --time=${walltime} --qos=serial --account=NRLMR03795YH2 --mem=0 srun --ntasks=1 ${script} "${script_arg}"
      ;;
  ...
  for template in "${SPACK_STACK_BATCH_TEMPLATES[@]}"; do
    ...
    case ${host} in
      atlantis)
        ...
        case ${compiler} in
          clang@=22.1.7)
            module use /gpfs/neptune/spack-stack/llvm-22.1.7/modulefiles
            module use /gpfs/neptune/spack-stack/openmpi-4.1.8/llvm-22.1.7/modulefiles
            ;;
          gcc@=13.4.0)
            ...
            ;;
          oneapi@=2026.0.0)
            module use /gpfs/neptune/spack-stack/oneapi-2026.0.0/modulefiles
            ;;
        esac
        ;;
      blueback)
      ...
      narwhal)
        ...
        case ${compiler} in
          oneapi@=2026.1.0)
            module use /p/app/projects/NEPTUNE/spack-stack/oneapi-2026.1.0/modulefiles
            ;;
        esac
        ;;
      nautilus)
    ...
    if [[ "${update_bootstrap_mirror}" == "true"*  ]]; then
      ...
      spack bootstrap mirror --binary-packages ${tmp_bootstrap_mirror_path} 2>&1 | tee log.bootstrap-mirror
    ...
    if [[ ! ${env_exists} == "true" ]]; then
      spack stack create env --name=${env_name} \
                             ...
                             2>&1 | tee log.create.${env_name}
    fi
    spack env activate -p ${env_dir}

    # Workaround for ParallelWorks (no NRL Enterprise GitHub access yet)
    # and other environment modifications (no qt@5 on DoW IronBank RHEL)
    case ${host} in
      navy-aws)
        sed -i 's/+adp/~adp/g' ${env_dir}/spack.yaml
        sed -i 's/+sdp/~sdp/g' ${env_dir}/spack.yaml
        sed -i 's/+jedi/~jedi/g' ${env_dir}/spack.yaml
        sed -i 's/+lis-ncoda-utils/~lis-ncoda-utils/g' ${env_dir}/spack.yaml
        ;;
      nrl-rhel9)
        sed -i 's/- ewok-env/- ewok-env ~ecflow/g' ${env_dir}/spack.yaml
        sed -i 's/~ecflow ~ecflow/~ecflow/g' ${env_dir}/spack.yaml
        ;;
    esac
    ...
    if [[ "${update_build_cache}" == "true" ]]; then
      # A value of 200 breaks gobject-introspection on Nautilus:
      # the path to the Python interpreter is cut off at p(ython)
      spack config add config:install_tree:padded_length:190
    fi

    # Bootstrap spack explicitly
    echo "Bootstrapping spack ..."
    spack bootstrap now 2>&1 | tee log.bootstrap.${env_name}

    # Concretize environment, and check that spack.lock is created
    spack concretize --force --fresh 2>&1 | tee log.concretize.${env_name}
...
echo "Running spack install for '\${arg}' on \$(hostname)"

# On Atlantis, need to add path to git/git-lfs on compute nodes
if [[ "${host}" == "atlantis" ]]; then
  export PATH=/software7/depot/git-2.38.1/bin:/software7/depot/git-2.38.1/libexec/git-core:\${PATH}
  git --version
  git lfs --version
fi

# On Nautilus, need to unload slurm so that spack can load it (part of site config)
if [[ "${host}" == "nautilus" ]]; then
  set +e
  module unload slurm
  set -e
fi
...
if [[ "\${arg}" == "qtdeps" ]]; then
  ...
  set -o pipefail
  spack install --verbose ${buildcache_install_flags} ${parallel_install_flags} --only=dependencies qt \\
    2>&1 | tee log.install.${env_name}.qt-dependencies
  set +o pipefail

elif [[ "\${arg}" == "ecflowdeps" ]]; then

  set -o pipefail
  spack install --verbose ${buildcache_install_flags} ${parallel_install_flags} --only=dependencies ecflow \\
    2>&1 | tee log.install.${env_name}.ecflow-dependencies
  set +o pipefail
  ...
  if [[ \${#test_packages[@]} -eq 0 ]]; then
    set -o pipefail
    spack install --verbose ${buildcache_install_flags} ${parallel_install_flags} 2>&1 | tee log.install.${env_name}
    set +o pipefail
  else
    for (( idx=0; idx<\${#test_packages[@]}; idx++ )); do
      test_package=\${test_packages[\${idx}]}
      # First, check if this package is in this environment
      grep -e "\${test_package}@" log.concretize.${env_name} || continue
      set -o pipefail
      spack install --verbose ${buildcache_install_flags} ${parallel_install_flags} --only=dependencies \${test_package} \\
        2>&1 | tee log.install.${env_name}.\${test_package}-dependencies
      spack install --verbose --no-cache --test=root \${test_package} 2>&1 | tee log.install.${env_name}.\${test_package}
      set +o pipefail
    done
    set -o pipefail
    spack install --verbose ${buildcache_install_flags} ${parallel_install_flags} 2>&1 | tee log.install.${env_name}
  ...
  echo "Error, argument '\${arg}' unknown"
  exit 1
...
EOF
    ...
    if [[ "${submit_to_scheduler}" == "true" ]]; then
      if grep -q "qt@" "log.concretize.${env_name}"; then
        run_interactive_job ${host} ${install_script} "qtdeps"
        spack install --verbose ${buildcache_install_flags} --jobs=1 qt 2>&1 | tee log.install.${env_name}.qt
      fi
      if grep -q "ecflow@" "log.concretize.${env_name}"; then
        run_interactive_job ${host} ${install_script} "ecflowdeps"
        spack install --verbose ${buildcache_install_flags} --jobs=1 ecflow 2>&1 | tee log.install.${env_name}.ecflow
      fi
      run_interactive_job ${host} ${install_script} "all"
    else
    ...
    if [[ "${update_build_cache}" == "false" ]]; then
      spack module ${module_choice} refresh --yes --upstream-modules 2>&1 | tee log.modules.${env_name}
      spack stack setup-meta-modules 2>&1 | tee log.setup-meta-modules.${env_name}
    fi
